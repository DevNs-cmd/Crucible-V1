...
import * as MeetingsService from '../crm/meetings.service';
...
export async function getMeetings(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    sendSuccess(res, meetings, 'Meetings fetched');
  } catch (err) { next(err); }
}
...
export async function createMeeting(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const meeting = await MeetingsService.createMeeting(req.params['id']!, req.user!.userId, parsed.data);
    sendSuccess(res, meeting, 'Meeting logged', 201);
  } catch (err) { next(err); }
}
